...
def ILPF(input_image,D0):
    ...
    output_image=np.zeros([H,W],dtype=np.float32)#int32

    # pad input image and DFT
    P=2*H
    Q=2*W
    padimage=np.zeros([P,Q],dtype=np.float32)#int32
...
if __name__ == '__main__':
    ...
    out1=ILPF(img,460)
    cv2.imshow('Q5_2_D0=460',out1)
    cv2.waitKey(0)
    savedimage1=Image.fromarray(out1)
    savedimage1.save('Q5_2_D0=460.tif')